...
**Execução deste ciclo:** etapas 1, 2 e 4 concluídas; etapa 3 entregue no código, faltando aprovação visual e medição de FPS. A etapa 5 teve a parte decidida implementada (horas jogadas, desempate e Level S) e o restante virou proposta no [documento 20](./20_contrato_rating_e_eventos.md). A etapa 6 depende dessa aprovação.
...
**Próxima ação:** o proprietário analisar o [documento 20](./20_contrato_rating_e_eventos.md),
que propõe a fórmula do Rating, o ELO, o contrato de eventos do plugin, a
retenção e a matriz OWNER/ADMIN. Sem essa aprovação a etapa 6 não começa, porque
o ranking público dependeria de números não revisados. Seguem também pendentes a
