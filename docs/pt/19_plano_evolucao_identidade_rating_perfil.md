...
**Execução deste ciclo:** etapas 1, 2, 4 e 5 concluídas; etapa 3 entregue no código, faltando aprovação visual e medição de FPS. O ciclo competitivo está fechado de ponta a ponta — o plugin emite rounds, a API ingere, as unidades fecham e a calibração avança. Falta rodar com jogo real e recalibrar as referências pela rotina de medição.
...
chegam, as unidades fecham e a calibração avança com jogo de verdade. Acumulados
rounds suficientes, rodar `ReferenceCalibrationService.report("RETAKE")` e decidir
sobre publicar referências medidas numa versão nova do algoritmo. Só depois disso
a etapa 6 — rankings por modo, níveis coloridos e S visível — faz sentido, porque
ela exibe números que hoje ainda não foram calibrados. Seguem pendentes a
aprovação visual e a medição de FPS da etapa 3, e o dropdown do avatar.
...
| 05/09/2026 | Referências viraram medida, não estimativa | `ReferenceCalibrationService` calcula as referências observadas a partir dos rounds gravados e as compara com as embarcadas, mostrando o desvio. Lê só rounds já absorvidos por unidade fechada, exige 2.000 rounds por lado antes de publicar qualquer número — uma média sobre poucos rounds seria tão arbitrária quanto o chute que ela substitui — e **não altera nada**: publicar é editar `CompetitiveScales` e subir a versão, porque trocar os números por baixo invalidaria silenciosamente todo Rating já calculado. Isso encerra a pendência das referências como preocupação em aberto e a transforma em procedimento com evidência. Validação: 173 unitários + 71 integrações verdes |
