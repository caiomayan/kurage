...
**Execução deste ciclo:** etapas 1, 2 e 4 concluídas; etapa 3 entregue no código, faltando aprovação visual e medição de FPS. A etapa 5 está implementada no backend — modelo competitivo, ingestão de rounds, horas jogadas, desempate e Level S — e o [documento 20](./20_contrato_rating_e_eventos.md) foi aprovado. Falta o plugin emitir os eventos, e as referências numéricas continuam pendentes de recalibração.
...
**Próxima ação:** fazer o plugin `Kurage.Core` emitir os eventos de round para
`POST /plugin/v1/servers/{id}/rounds`. É a última peça que falta para a
calibração avançar com jogo real; sem ela o modelo competitivo existe e é
testado, mas não recebe entrada. Depois disso, recalibrar as referências de
`CompetitiveScales` sobre os primeiros rounds reais antes de qualquer exibição
pública. Seguem pendentes a aprovação visual e a medição de FPS da etapa 3, e o
dropdown do avatar.
