...
import { PiArrowDownRight, PiWaves } from "react-icons/pi";
import { cn } from "@/lib/utils";
import { CS2_CONNECT_HOST } from "@/lib/constants";
...
export function MergulharButton({
  ...
  const canConnect = cleanIp.length > 0;
  ...
  return (
    <div className={cn("relative inline-flex group select-none", className)}>
      ...
      <div 
        ...
        style={{
          background: "radial-gradient(ellipse at center, rgba(var(--kurage-accent-rgb),0.4) 0%, rgba(146, 188, 227, 0.2) 60%, transparent 80%)"
        }}
      />
      ...
      <button
        ...
        onClick={handleClick}
        disabled={!canConnect}
        title={canConnect ? `Conectar em ${cleanIp}` : "Servidor ainda não configurado"}
        ...
        className={cn(
          ...
          "border border-[rgba(var(--kurage-accent-rgb),0.35)] hover:border-[rgba(var(--kurage-accent-rgb),0.85)]",
          "bg-[#05090c]/90 backdrop-blur-xl shadow-[0_4px_24px_rgba(0,0,0,0.8)] disabled:cursor-not-allowed disabled:opacity-50",
      ...
      >
        ...
        >
          ...
          <div 
            ...
            style={{
              background: "linear-gradient(180deg, rgba(var(--kurage-accent-rgb),0.25) 0%, rgba(146, 188, 227, 0.45) 100%)"
            }}
          />
          ...
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_120%,_rgba(var(--kurage-accent-rgb),0.4)_0%,_transparent_65%)]" />
        </div>

        {/* ── 4. WATER SURFACE PERIMETER HIGHLIGHT ── */}
        <div className="absolute top-0 left-0 right-0 h-[1px] bg-gradient-to-r from-transparent via-[rgba(var(--kurage-accent-rgb),0.8)] to-transparent opacity-70 group-hover:opacity-100 transition-opacity duration-300" />
        ...
        <div className="relative z-10 flex items-center justify-center">
          ...
          <div className="relative w-4 h-4 flex items-center justify-center text-[var(--kurage-accent)] transition-all duration-300 group-hover:text-white group-hover:translate-y-0.5 group-hover:scale-110">
            <PiWaves className="w-3.5 h-3.5 absolute inset-0 m-auto transition-opacity duration-300 group-hover:opacity-0" />
            <PiArrowDownRight className="w-3.5 h-3.5 absolute inset-0 m-auto opacity-0 transition-all duration-300 group-hover:opacity-100 text-[var(--kurage-accent)]" />
          </div>
        </div>
        ...
        {isPressed && (
          <motion.div
            ...
            className="absolute inset-0 rounded-full bg-[rgba(var(--kurage-accent-rgb),0.6)] pointer-events-none mix-blend-screen"
          />
        )}
